  ...
  onIconClick = () => {};
	...
	currentElement = (processingElements) => {
	  if (processingElements && processingElements.length) {
	    ...
	    return new SimpleFigureInstance(this.generateId(), this.code(), this.svgImage(), this.width(), this.height(),
	      this.defaultColor())
	  }
	};
	...
	canBeMounted = (currentElements, element, fixedX, fixedY) => {
	  return currentElements.filter(e => e.code() === 'Pipeline').some(p => p.containsPoint(fixedX, fixedY)) &&
					 currentElements.filter(e => e.code() !== 'Pipeline').every(f => !f.figuresIntersects(element))
	};
	...
	deserialize = (data) => {
	  let instance = new SimpleFigureInstance(data.id, this.code(), this.svgImage(), this.width(), this.height(), data.color)
...
class SimpleFigureInstance {
	...
	constructor(id, codeId, svgImage, width, height, color, canBeMounted = true) {
	  this.id = id
	  this.codeId = codeId
	  this.position = null
	  this.color = color
	...
	render = () => {
	  return <SimpleFigureElement key={this.id}
...
class SimpleFigureElement extends React.Component {
  ...
  <rect width={7} height={7} x={-offsetX - 1.5} y={offsetY + 0.5}
  style={{'fill': '#FFFFFF', 'stroke': '#DDDFE1', 'strokeWidth': '1', 'fillOpacity': '1', 'strokeOpacity': '0.9'}} />
  <rect width={7} height={3} x={-offsetX - 1.5} y={offsetY + 0.5}
  style={{'fill': '#EFF6F9', 'stroke': '#DDDFE1', 'strokeWidth': '1', 'fillOpacity': '1', 'strokeOpacity': '0.9'}} />
  <text x={-offsetX + 0.6} y={offsetY + 2.5} style={{'fill': '#000000', fontSize: '1.5px'}}>{this.props.labelText}</text>
	    </g>
	  )
	};
}
